...
|-------------|---------|-----------|----------|-----------|------------|
| **Local/Mock** | local | quckapp-mock | 1 | 256Mi/0.25CPU | No |
| **Dev** | aks-dev | quckapp | 2 | 512Mi/0.5CPU | No |
| **QA** | aks-qa | quckapp-qa | 2 | 512Mi/0.5CPU | No |
| **UAT1** | aks-uat | quckapp-uat1 | 2 | 1Gi/1CPU | No |
| **UAT2** | aks-uat | quckapp-uat2 | 2 | 1Gi/1CPU | No |
| **UAT3** | aks-uat | quckapp-uat3 | 3 | 1Gi/1CPU | No |
| **Staging** | aks-staging | quckapp-staging | 3 | 2Gi/2CPU | Yes (3-10) |
| **Live** | aks-prod | quckapp-prod | 5 | 4Gi/4CPU | Yes (5-50) |
...
variables:
  - group: QuckApp-Global-Variables
  - name: acrRegistry
    value: 'quckapp.azurecr.io'
  ...
  - stage: Deploy_Local_Mock
    ...
    jobs:
      - deployment: DeployLocalMock
        ...
        environment: 'quckapp-local-mock'
        pool:
            ...
            preDeploy:
              steps:
                - task: ManualValidation@0
                  ...
                  inputs:
                    notifyUsers: |
                      product-owner@quckapp.com
                      team-a-lead@quckapp.com
            ...
            preDeploy:
              steps:
                - task: ManualValidation@0
                  ...
                  inputs:
                    notifyUsers: |
                      release-manager@quckapp.com
                      devops-team@quckapp.com
                    instructions: |
              ...
              steps:
                - template: templates/deploy-steps.yml
                  parameters:
                    ...
                    resourceGroup: 'rg-quckapp-staging'
      ...
      - job: NotifyStakeholders
        ...
        steps:
          - task: SendEmail@1
            ...
            inputs:
              To: 'stakeholders@quckapp.com'
              Subject: 'QuckApp Deployment: ${{ parameters.serviceName }} deployed to Production'
              Body: |
                Deployment Complete
...
steps:
  ...
  - task: Kubernetes@1
    ...
    inputs:
      ...
      azureSubscriptionEndpoint: 'QuckApp-${{ parameters.environment }}'
  ...
  - task: AzureCLI@2
    ...
    inputs:
      ...
      scriptLocation: 'inlineScript'
      inlineScript: |
        SERVICE_URL="https://${{ parameters.serviceName }}.${{ parameters.environment }}.quckapp.dev"
...
steps:
    ...
    inputs:
      ...
      inlineScript: |
        ...
        kubectl rollout undo deployment/${{ parameters.serviceName }} \
          -n quckapp-${{ parameters.environment }}
  ...
  - task: SendEmail@1
    ...
    inputs:
      To: 'ops-team@quckapp.com;release-manager@quckapp.com'
...
```yaml
# Variable Group: QuckApp-Global-Variables
variables:
  acrRegistry: 'quckapp.azurecr.io'
  aksSubscription: 'xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx'
  keyVaultName: 'kv-quckapp'
...
```yaml
# Variable Group: QuckApp-Variables
variables:
  aksCluster: 'aks-dev'
  resourceGroup: 'rg-quckapp'
  namespace: 'quckapp'
  dbConnectionString: '$(DEV_DB_CONNECTION)'
  redisHost: 'redis-dev.quckapp.internal'
  kafkaBrokers: 'kafka-dev.quckapp.internal:9092'

# Variable Group: QuckApp-Live-Variables
variables:
  aksCluster: 'aks-prod'
  resourceGroup: 'rg-quckapp-prod'
  namespace: 'quckapp-prod'
  dbConnectionString: '$(PROD_DB_CONNECTION)'
  redisHost: 'redis-prod.quckapp.internal'
  kafkaBrokers: 'kafka-prod.quckapp.internal:9092'
```
...
|------------|------|-------|
| `QuckApp-local` | Azure Resource Manager | Local subscription |
...
```yaml
# quckapp-production environment configuration
checks:
  - type: approvals
    settings:
      approvers:
